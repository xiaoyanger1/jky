...
namespace SysDetection.Detection
{
    public partial class ComplexAssessment : Form
    {
        ...
        /// 检验项目
        /// </summary>
        public SysDetection.Common._Public_Enum.ENUM_DetectionItem? enum_DetectionItem = null;
        ...
        private bool GetItem(string jyxm)
        {
            if (jyxm == "气密性能检测")
            {
                enum_DetectionItem = SysDetection.Common._Public_Enum.ENUM_DetectionItem.enum_气密性能检测;
                return true;
            }
            else if (jyxm == "水密性能检测")
            {
                enum_DetectionItem = SysDetection.Common._Public_Enum.ENUM_DetectionItem.enum_水密性能检测;
                return true;
            }
            else if (jyxm == "气密性能及水密性能检测")
            {
                enum_DetectionItem = SysDetection.Common._Public_Enum.ENUM_DetectionItem.enum_气密性能及水密性能检测;
                return true;
            }
            else
            {
        ...
        private void InitResult()
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                if (i == 0)
                {
                    ...
                    if (enum_DetectionItem == SysDetection.Common._Public_Enum.ENUM_DetectionItem.enum_气密性能检测)
                    {
                    ...
                    else if (enum_DetectionItem == SysDetection.Common._Public_Enum.ENUM_DetectionItem.enum_水密性能检测)
                    {
                    ...
                    else if (enum_DetectionItem == SysDetection.Common._Public_Enum.ENUM_DetectionItem.enum_气密性能及水密性能检测)
                    {
                        lbl_1desc.Text = dt.Rows[i]["sm_Remark"].ToString();
                        lbl_1resdesc.Text = dt.Rows[i]["sm_PaDesc"].ToString();
                        txt_1fy.Text = dt.Rows[i]["sm_Pa"].ToString();
        ...
        private int Get_SMLevel(DataTable dt)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                if (dt.Rows.Count == 3)
                {
                    ...
                    int minlevel = new SysDetection.model.QM_Dict.Dict().GetList().Find(t => t.value == min).level,
                        intermediatelevel = new SysDetection.model.QM_Dict.Dict().GetList().Find(t => t.value == intermediate).level,
                        maxlevel = new SysDetection.model.QM_Dict.Dict().GetList().Find(t => t.value == max).level;

                    if ((maxlevel - intermediatelevel) > 2)
                    {
                        max = new SysDetection.model.QM_Dict.Dict().GetList().Find(t => t.level == (intermediatelevel + 2)).value;
                    }
        ...
        /// type 1正压2负压
        /// <returns></returns>
        private int Get_QMLevel(DataTable dt, int type)
        {
            qm_z_FC = 0; qm_f_FC = 0; qm_z_MJ = 0; qm_f_MJ = 0;

            for (int i = 0; i < dt.Rows.Count; i++)
            {
                //if (double.Parse(dt.Rows[i]["qm_Z_FC"].ToString()) > qm_z_FC)
                //{
                //    qm_z_FC += double.Parse(dt.Rows[i]["qm_Z_FC"].ToString());
                //}

                //if (double.Parse(dt.Rows[i]["qm_F_FC"].ToString()) > qm_f_FC)
                //{
                //    qm_f_FC += double.Parse(dt.Rows[i]["qm_F_FC"].ToString());
                //}


                //if (double.Parse(dt.Rows[i]["qm_Z_MJ"].ToString()) > qm_z_MJ)
                //{
                //    qm_z_MJ += double.Parse(dt.Rows[i]["qm_Z_MJ"].ToString());
                //}

                //if (double.Parse(dt.Rows[i]["qm_F_MJ"].ToString()) > qm_f_MJ)
                //{
        ...
        private void btn_audit_Click(object sender, EventArgs e)
        {
            ...
            if (enum_DetectionItem == SysDetection.Common._Public_Enum.ENUM_DetectionItem.enum_气密性能检测 || enum_DetectionItem == SysDetection.Common._Public_Enum.ENUM_DetectionItem.enum_气密性能及水密性能检测)
            {
                for (int i = 0; i < con; i++)
                {
            ...
            if (enum_DetectionItem == SysDetection.Common._Public_Enum.ENUM_DetectionItem.enum_水密性能检测 || enum_DetectionItem == SysDetection.Common._Public_Enum.ENUM_DetectionItem.enum_气密性能及水密性能检测)
            {
                for (int i = 0; i < con; i++)
                {
            ...
            if (enum_DetectionItem == SysDetection.Common._Public_Enum.ENUM_DetectionItem.enum_气密性能检测 || enum_DetectionItem == SysDetection.Common._Public_Enum.ENUM_DetectionItem.enum_气密性能及水密性能检测)
            {
                txt_dj1.Text = Get_QMLevel(dt, 1).ToString();
                txt_dj3.Text = Get_QMLevel(dt, 2).ToString();
                ...
                if (qm_f_MJ <= double.Parse(txt_sjz5.Text))
                {
                    txt_jg5.Text = "合格";
                }
                else
